...
def lambda_handler(event,context):
    ...
    print(f'Form_Data : Output_after_preprocessing_data : {enrollment_data}.')
    try: 
        json_data = data_postprocessing(enrollment_data)
        print(f'Form_Data : Postprocessed_model_output : {json_data}.')
        enrollment_json_object = json.loads(json_data)
        print(f'Form_Data : json_output_of_model : {enrollment_json_object}.')
        sqs_data = SQS_data_postprocessing(event,enrollment_json_object)
        print(f'Form_Data : Data_to_be_sent_to_SQS_after_postprocessing_json_output_of_model : {sqs_data}.')
        SQS_Publisher(sqs_data)
        print(f'Form_Data : Data_sent_to_sqs_sucessfully.')
        return {"statusCode": 200,"body": "Data Sent to SQS Sucessfully"}
    except Exception as e:
        enrollment_json_object = json.loads('{}')
        print(f'Form_Data : Json_output_of_model : {enrollment_json_object}.')
        sqs_data = SQS_data_postprocessing(event,enrollment_json_object)
        print(f'Form_Data : Data_to_be_sent_to_SQS_after_postprocessing_json_output_of_model : {sqs_data}.')
        SQS_Publisher(sqs_data)
